...
The mirror pipeline ingests the public crates.io index and downloads corresponding crate artifacts. It pairs each crate with a lightweight metadata sidecar, maintains a JSONL manifest for auditing/resume, and can bundle artifacts into rolling TAR archives.
...
- Download-Crates (Go)
  ...
  - Exposes Prometheus metrics and pprof endpoints when `-listen` is specified.
  - Key flags: `-index-dir`, `-out`, `-concurrency`, `-include-yanked`, `-limit`, `-bundle`, `-bundles-out`, `-bundle-size-gb`, `-manifest`, `-checksums`, `-retries`, `-retry-base`, `-retry-max`, `-log-format`, `-log-level`, `-progress-interval`, `-progress-every`, `-listen`.
...
The downloader exposes Prometheus metrics and pprof endpoints when started with `-listen :PORT`:

- `/metrics` - Prometheus scrape endpoint with 6 metrics (requests, bytes, duration, retries, inflight, processed)
- `/api/status` - JSON status summary for dashboards
- `/debug/pprof/*` - Go profiling endpoints

See [Prometheus.md](Prometheus.md) for full metric documentation.
...
- Sidecars capture original index metadata for offline verification.
- JSONL manifest provides an auditable record of all download attempts and outcomes.
